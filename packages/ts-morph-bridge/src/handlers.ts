...
export function handleInitProject(params: Record<string, unknown>): {
  ok: true;
  files: string[];
} {
...
export function handleRenameSymbol(params: Record<string, unknown>): object {
  ...
  const scopeNote = scopeNode ? ` within scope '${scopeNode}'` : " (file-wide)";
  const summary =
...
export function handleMoveSymbolToFile(
  params: Record<string, unknown>
): object {
  const p = requireProject();
  ...
  return {
    summary:
      `Moved '${symbolName}' from ` + `${sourceFilePath} to ${targetFilePath}`,
    changed_files: getChangedFilesList(p),
  };
}
...
export function handleGetDiagnostics(
  ...
  const rawDiags = filePath
    ? p
        ...
        .filter((d) => d.getSourceFile()?.getFilePath() === filePath)
