...
function parseRenameIntentFromPrompt(prompt) {
    const trimmed = prompt.trim();
    try {
        const data = JSON.parse(trimmed);
        if (typeof data?.old_name === "string" &&
    ...
    const toMatch = trimmed.match(/rename\s+(\S+)\s+to\s+(\S+)/i) ||
...
function getGlobPatterns(engine) {
    if (engine === "typescript") {
        return ["**/*.ts", "**/*.tsx"];
    }
    return ["**/*.py"];
}
async function gatherWorkspaceFiles(workspaceFolder, engine) {
    const files = [];
    const patterns = getGlobPatterns(engine);
    for (const pattern of patterns) {
        const uris = await vscode.workspace.findFiles(new vscode.RelativePattern(workspaceFolder, pattern), null, 2000);
...
class RefactorViewProvider {
    ...
    async runRefactor(globalState, folder, post, append, resume, promptText) {
        ...
        try {
            ...
            const intent = promptText != null
                ? parseRenameIntentFromPrompt(promptText)
                : null;
            if (!intent) {
                append("message", "Say something like: rename foo to bar, or paste JSON: {\"old_name\": \"foo\", \"new_name\": \"bar\"}");
                post("submitDone");
                return;
            }
