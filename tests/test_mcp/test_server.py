...
async def test_rename_symbol_in_file_success(tmp_path: Path) -> None:
    ...
    result = await _rename_symbol_in_file(
...
async def test_rename_symbol_in_file_not_found() -> None:
...
async def test_rename_symbol_in_file_invalid_syntax(tmp_path: Path) -> None:
    """Helper returns ERROR when file is not valid Python."""
    path = tmp_path / "bad.py"
    path.write_text("def foo( \n  syntax error", encoding="utf-8")
    result = await _rename_symbol_in_file(path, "foo", "bar", scope_node=None)
    assert "ERROR" in result
    assert "syntax" in result.lower()


async def test_rename_symbol_in_file_symbol_not_found(
    tmp_path: Path,
) -> None:
    """Helper returns ERROR when symbol is not in file."""
    path = tmp_path / "code.py"
    path.write_text("def foo(): pass\n", encoding="utf-8")
    result = await _rename_symbol_in_file(path, "bar", "baz", scope_node=None)
    assert "ERROR" in result
    assert path.read_text() == "def foo(): pass\n"


async def test_rename_symbol_in_file_with_scope(tmp_path: Path) -> None:
    ...
    result = await _rename_symbol_in_file(path, "x", "value", scope_node="outer")
    assert "Renamed" in result
