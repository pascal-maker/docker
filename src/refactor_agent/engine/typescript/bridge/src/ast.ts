...
export function addDeclarationToTarget(decl: Node, targetSf: SourceFile): void {
...
export function updateImportsForMove(
  ...
  for (const sf of p.getSourceFiles()) {
    ...
    const existing = sf
      .getImportDeclarations()
      .find((i) => i.getModuleSpecifierSourceFile() === targetSf);
    if (existing) {
      existing.addNamedImport(symbolName);
    } else {
      const specifier = sf.getRelativePathAsModuleSpecifierTo(targetSf);
