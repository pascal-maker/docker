...
**Relevant files:** The plan file(s) above, `src/refactor_agent/ast_refactor/` (engine, agent, mcp_server, a2a_executor, a2a_server), `apps/backend/scripts/mcp/run_ast_refactor_mcp.py`, `apps/backend/scripts/a2a/run_ast_refactor_a2a.py`, `README.md`, `pyproject.toml`, `CLAUDE.md`, `vision.md`.
...
- **A2A refactor server:** `uv run --directory apps/backend python scripts/a2a/run_ast_refactor_a2a.py` (default `http://localhost:9999`). It already supports:
  ...
  - **Bridge compatibility:** The server’s middleware already maps `tasks/send` → `message/send` and `tasks/getResult` → task result (see `apps/backend/scripts/a2a/run_ast_refactor_a2a.py`). So the bridge can speak A2A JSON-RPC to this server.
...
- **Protocol:** Bridge talks to the refactor server via HTTP (A2A JSON-RPC). Use the same request/response shape as in `apps/backend/scripts/a2a/run_ast_refactor_a2a.py` and `apps/backend/scripts/a2a/call_a2a_agent.py` (or the test scripts) so the existing server works unchanged.
