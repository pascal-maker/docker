# VS Code extension docs

Documentation for the Refactor Agent VS Code / Cursor extension (source in `vscode-extension/`).

- **[overview.md](overview.md)** — Flow, UI, configuration, and relation to Cursor/MCP.
- **[install.md](install.md)** — How to install from the repo or VSIX, and how to have the extension available by default in your dev environment.
- **[debugging.md](debugging.md)** — Debugging a blank panel or missing messages (webview DevTools, console logs, common errors).

Usage and configuration are in the extension’s own [README](../../../apps/vscode-extension/README.md).
