...
3. **Use a client** to talk to the agent:
   - **VS Code / Cursor:** Install the [Refactor Agent extension](vscode-extension/README.md); it speaks A2A HTTP directly.
   - **Cursor (MCP):** Configure the [A2A–MCP bridge](cursor-bridge.md) and point it at `http://localhost:9999`.

See [Docker deployment](docker-deployment.md) for details (LiteLLM proxy, env vars, running without Docker).
...
- **VS Code / Cursor:** Use the [Refactor Agent extension](vscode-extension/README.md). Configure the A2A base URL to match your server (e.g. `http://localhost:9999`).
- **Cursor only:** Use the [Cursor bridge](cursor-bridge.md): register the agent at your server URL, then use the bridge’s MCP tools to send refactor tasks.
...
Then open the Chainlit app (default localhost:8000). See [Chat UI (Chainlit)](chat-ui.md) for modes (Ask / Auto / Plan), persistence, and auth.
